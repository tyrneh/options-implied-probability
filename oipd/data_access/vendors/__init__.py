...
def get_reader(name: str):
    ...
    if name not in _READER_REGISTRY:
        raise ValueError(f"Unknown vendor '{name}'. Available: {list(_READER_REGISTRY)}")
...
def get_adapter(name: str) -> VendorAdapter:
    ...
    if name not in _ADAPTER_REGISTRY:
        raise ValueError(f"Unknown vendor '{name}'. Available: {list(_ADAPTER_REGISTRY)}")
    module = import_module(_ADAPTER_REGISTRY[name])
    adapter_cls = getattr(module, "YFinanceAdapter")  # TODO: generalize class name lookup
    return adapter_cls()
