...
from oipd.core.vol_surface_fitting.shared.svi_types import (
    SVICalibrationDiagnostics,
    SVICalibrationOptions,
)
from oipd.core.vol_surface_fitting.shared.vol_model import VolModel
from oipd.core.vol_surface_fitting.algorithms.ssvi.surface import SSVISliceObservations
    ...
    *,
    ...
    for obs in observations:
        ...
        if np.isnan(diagnostics.objective):
            diagnostics.objective = float(
                np.mean(
                    (
                        svi_total_variance(obs.log_moneyness, params)
                        - obs.total_variance
                    )
                    ** 2
                )
    ...
    calendar_margin, pair_margins, pair_deltas = _calendar_metrics(
        fitted_slices, grid=k_grid
    )
