...
def validate_export_domain(
    ...
    Args:
        domain: Optional lower/upper bound tuple.

    Returns:
        Optional[tuple[float, float]]: Normalized domain tuple or ``None``.
...
def resolve_surface_export_expiries(
    ...
    *,
    ...
    Returns:
        tuple[pd.Timestamp, ...]: Sorted, deduplicated export expiries.
...
def _resolve_export_bound(
    ...
    *,
    ...
    """Resolve and validate a single export bound against surface maturity rules.
    ...
    Returns:
        pd.Timestamp | None: Resolved bound timestamp when provided.
    ...
    if not isinstance(bound, (str, date, pd.Timestamp)):
        raise ValueError(
            "Surface export bounds must be date-like (str, datetime.date, or pandas.Timestamp)."
        )
...
def _normalize_timestamp(value: Any) -> pd.Timestamp:
    ...
    Returns:
        pd.Timestamp: Timezone-naive timestamp.
    """
