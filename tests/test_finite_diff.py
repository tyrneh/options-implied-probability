...
class TestFiniteDifferenceStability:
    ...
    def test_finite_diff_functionality_and_precision(self):
        ...
        assert (
            max_error < 1e-10
        ), f"Should be very accurate for polynomials: max_error = {max_error}"
        ...
        deep_otm_prices = np.maximum(
            0.01, 2.0 * np.exp(-0.05 * (deep_otm_strikes - spot))
        )
        ...
        realistic_prices = np.maximum(
            0.1,
            spot
            - realistic_strikes
            + 3.0 * np.exp(-0.01 * (realistic_strikes - spot) ** 2),
        )

        d2y_realistic = finite_diff_second_derivative(
            realistic_prices, realistic_strikes
        )
    ...
    def test_finite_diff_vs_gradient_comparison(self):
        ...
        option_prices = np.maximum(
            0.01, np.exp(-0.1 * (strikes - spot) ** 2)
        )  # Gaussian-like decay
        ...
        assert np.all(
            np.abs(d2y_new) < 1e6
        ), "New method should produce reasonable magnitudes"
    ...
    def test_finite_diff_maintains_pdf_properties(self):
        ...
        call_prices = np.maximum(
            0.1, spot - strikes + 10 * np.exp(-0.01 * (strikes - spot) ** 2)
        )
...
class TestIntegrationWithPDFCalculation:
    ...
    def test_pdf_calculation_with_synthetic_data(self):
        ...
        try:
            ...
            pytest.fail(
                f"PDF calculation failed with new finite difference method: {e}"
            )
    ...
    def test_pdf_numerical_stability_improvement(self):
        ...
        base_prices = np.maximum(
            0.5, spot - strikes + 15 * np.exp(-0.003 * (strikes - spot) ** 2)
        )
