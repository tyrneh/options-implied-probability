...
class TestModelParamsIntegration:
    ...
    def test_model_params_default_mid(self):
        """Test that ModelParams defaults to 'mid' price method."""
        model = ModelParams()
...
class TestEndToEndPriceMethod:
        ...
        return pd.DataFrame(
            {
                ...
                "ask": [
                    ...
                    4.0,
                    2.0,
                ],  # Mid would be [12.0, 8.0, 5.0, 3.0, 1.5]
            }
        )
    ...
    def test_price_methods_give_different_results(
        ...
        result_last = RND.from_dataframe(
            sample_options_data, market_inputs, model=model_last
        )
        result_mid = RND.from_dataframe(
            sample_options_data, market_inputs, model=model_mid
        )
    ...
    def test_backward_compatibility_default_mid(
        self, sample_options_data, market_inputs
    ):
        """Test that default behavior (no explicit price_method) uses mid price when bid/ask available."""
        # No explicit model - should default to mid price
        ...
        # Explicit mid price
        model_mid = ModelParams(price_method="mid", pricing_engine="bs")
        result_explicit_mid = RND.from_dataframe(
            sample_options_data, market_inputs, model=model_mid
        )

        # Results should be identical
        assert np.array_equal(result_default.pdf, result_explicit_mid.pdf)
        assert np.array_equal(result_default.cdf, result_explicit_mid.cdf)
...
class TestRobustDataHandling:
    ...
    def test_mid_price_with_no_bid_ask_data(self):
        ...
        pd.testing.assert_series_equal(
            result["price"], df["last_price"], check_names=False
        )
