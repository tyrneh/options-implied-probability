...
class TestProbSurfaceDensityResults:
    ...
    def test_density_results_respects_hard_date_bounds(self, prob_surface):
        """start/end act as hard bounds on output rows."""
        result = prob_surface.density_results(
            start="2025-02-25",
            end="2025-05-01",
            step_days=7,
    ...
    def test_density_results_supports_risk_neutral_and_physical_views(
        self, prob_surface
    ):
        """Active measure is preserved in exported density values."""
        physical_frame = prob_surface.density_results(
            start=prob_surface.expiries[0], end=prob_surface.expiries[0]
        )
        rn_surface = prob_surface.to_risk_neutral()
        rn_frame = rn_surface.density_results(
            start=rn_surface.expiries[0], end=rn_surface.expiries[0]
        )

        assert list(physical_frame.columns) == list(rn_frame.columns)
        assert not np.allclose(
            physical_frame["pdf"].to_numpy(dtype=float),
            rn_frame["pdf"].to_numpy(dtype=float),
        )
